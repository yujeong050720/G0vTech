# G0vTech — IPFS 기반 탈중앙화 처방전 관리 및 Private AI 복약 안내 실험

가상 처방전을 암호화해 실제 IPFS에 저장하고, 암호화 파일의 CID를 Sepolia 블록체인에 등록하는 실험입니다. 작은 학습 모델은 CKKS 동형암호로 암호화된 입력을 계산해 안내 문장을 출력합니다.

## 웹에서 체험하기

https://medical-blockchain-demo.onrender.com/

운영자에게 받은 체험 코드를 입력하고 가상약A 또는 가상약B를 선택한 뒤 **전체 과정 실험하기**를 누르세요.

- 하루 전체 이용자 합계 20회, 동시에 한 요청만 처리합니다.
- 무료 서버가 잠든 경우 첫 접속에 약 1분 이상 걸릴 수 있습니다.
- 실제 처방전이나 개인정보는 입력하지 마세요.

## 처리 흐름

1. 가상 처방전 → AES-GCM 암호화 → 실제 Pinata IPFS 업로드
2. 암호화 파일의 CID와 무작위 기록 ID → 실제 Sepolia 원장 등록
3. 처방 문장 → 입력 벡터 → CKKS 암호화 → 별도 프로세스에서 모델 계산
4. 계산 결과 복호화 → 토큰 선택 → 안내 문장 출력

| 구분 | 로컬 버전 | 웹 버전 |
| --- | --- | --- |
| 실행 파일 | hospital_demo.py / app.py | web_demo.py |
| 처리 기록 저장 | 로컬 SQLite | Supabase |
| CKKS 암호화·복호화 | 사용자 PC | 웹 서버 |
| 암호문 AI 계산 | PC의 별도 프로세스 | 서버의 별도 프로세스 |

웹 입력은 HTTPS로 서버에 전달된 뒤 CKKS로 암호화됩니다. 브라우저에서부터 동형암호화하는 구조는 아닙니다.

모델은 두 가지 가상 처방 문장을 학습한 초소형 조건부 토큰 모델입니다. 범용 LLM이나 검증된 의료 모델이 아니며 실제 복약 판단에 사용할 수 없습니다.

배포 방법은 [WEB_DEPLOY.md](WEB_DEPLOY.md)를 참고하세요.
...
로컬 검증에서 웹 테스트를 포함한 34개 테스트가 모두 통과했습니다.
...
`verified`는 복호화한 모델 점수와 평문 기준 계산의 수치 검증 결과이며, 의료적 정확성을 뜻하지 않습니다.ㅎ
...
- CLI는 사용자 PC에서, 웹 버전은 서버에서 CKKS 암호화·복호화를 수행합니다. 암호문 모델 계산은 각각 별도 프로세스에서 실행하며, OS 권한으로 격리된 보안 경계를 구현한 것은 아닙니다.
